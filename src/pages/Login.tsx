...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 8) {
      toast({ variant: 'destructive', title: 'Erro', description: 'Credenciais inválidas. A senha deve ter no mínimo 8 caracteres.' })
      setIsLoading(false)
      return
    }
    ...
    if (!userRole) {
      toast({ variant: 'destructive', title: 'Erro', description: 'Email não cadastrado ou credenciais inválidas.' })
      setIsLoading(false)
      return
    }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4 animate-fade-in">
      ...
      <Card className="w-full max-w-md shadow-lg border-border/50">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Acesse o CRM utilizando suas credenciais.
          </CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <a 
                  ...
                  onClick={(e) => { e.preventDefault(); toast({ title: 'Recuperação de Senha', description: 'Link de recuperação enviado para seu email (simulação).' }) }}
                >
                  Esqueci minha senha
                </a>
              </div>
            ...
            <div className="text-xs text-muted-foreground p-3 bg-muted/50 rounded-md border border-border/50">
              <p className="font-semibold mb-1 text-foreground">Contas de teste (senha > 8 chars):</p>
